...
site <- 'oh_fm1' # site abbreviation that may be used for file naming conventions
...
site_tz <- 'America/New_York' # all data should be converted (or called into R) to the same time zone. 
...
date_format <- "%mm/%dd/%YY" # format of date variables in provided data files. For example, the default in Excel is
...
######## BMP timeline ############
start_date <- as.POSIXct('2012-10-02 03:57:00') # YYYY-MM-DD HH:MM:SS date/time of study start
                             # This can be the first second of the day on the date of the first sampling event
bmp_date <- as.POSIXct('2016-10-20 12:00:00') # YYYY-MM-DD HH:MM:SS date/time of BMP implementation. All events before
                           # this date will be considered 'before', all events after this 
                           # date will be considered 'after'
end_date <- as.POSIXct('2018-09-30 23:59:59') # YYYY-MM-DD HH:MM:SS date/time of study end

######## storm event data #############
## set wq data parameters (before-after site)
wq_file <- 'MaumeeR_WY13_18.csv' # file name (with .csv extention) where event-level water quality data are stored
...
flags <- NA #c('flag') # this can either be a vector of verbatim flag
...
######## rain data ###########
rain_site <- NA #'405051083391201' # site number for rain gage if pulling from NWIS, 'NA' if providing data file
rain_file <- 'AQ_precip_405051083391201.csv' # filename of rain file, 'NA' if pulling from NWIS. Should be a csv and include ".csv" at end of file name.

######## field activity data #########
activity_file <- 'Field_Management_data_OH.csv' # filename of field activity log and should include .csv extension
...
discharge_site_no <- '04188496' # if not providing a discharge file, the USGS stream gage site to pull discharge data from
...
noaa_site <- 'WBAN14825' # nearest NOAA met station site number. Can use both weather_file and noaa_site if 
